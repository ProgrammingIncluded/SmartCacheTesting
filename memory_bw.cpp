...
void measure_read_bandwidth(){
  ...
  std::chrono::duration<double> bandwidth_duration = bandwidth_end-bandwidth_start;
  std::chrono::duration<double> loop_overhead      = loop_end-loop_start;
  std::chrono::duration<double> bandwidth_time     = bandwidth_duration - loop_overhead;
  std::cout<<"The read bandwidth for size "<<size*64<<" Bytes over "<<repetitions<<" reps is "<<
  (repetitions*size*64)/bandwidth_time.count()<<" Bytes/Second\n";
  std::cout<<"Actual timing:"<<bandwidth_duration.count()<<"; Loop overhead:"<<loop_overhead.count()<<std::endl;
}
