...
void measure_latency(){
  ...
  auto page_fault_start = std::chrono::high_resolution_clock::now();
  for(i=0;i<size;i++){
    next = (page_t*)mmap(NULL, 4096, PROT_READ, MAP_PRIVATE, fd, next->next);
  }
  auto page_fault_end = std::chrono::high_resolution_clock::now();

  std::chrono::duration<double> page_fault_overhead = (page_fault_end-page_fault_start)-(loop_end-loop_start);
  std::cout<<"The page fault service time for array of size "<<size*4096<<" Bytes is "<<
  page_fault_overhead.count()*1000000000/size<<" ns\n";
